...
test.describe('Path Editing Advanced Tests', () => {
  test('should convert path commands', async ({ page }) => {
    ...
    const convertButton = page.locator('button', { hasText: 'Change to' });
    ...
    if (convertButtonExists) {
      ...
      const convertButtonAfter = page.locator('button', { hasText: 'Change to' });
  ...
  test('should split subpaths', async ({ page }) => {
    ...
    for (const point of pointsToTry) {
      ...
      const cutButton = page.locator('button', { hasText: 'Cut Subpath' });
      if (await cutButton.count() > 0) {
        ...
        const cutButtonAfter = page.locator('button', { hasText: 'Cut Subpath' });
