 ...
 */
export const SnapPointCrossOverlay: React.FC<SnapPointCrossOverlayProps> = ({
  snapPoints,
  viewport,
  opacity = 0.5,
  crossSize,
  ...
  return (
    <g className="snap-point-crosses">
      {snapPoints.map((snap, index) => {
        ...
        return (
          <g key={`snap-cross-${snap.elementId}-${index}`} opacity={opacity}>
            {/* Horizontal line */}
            <line
              x1={x - size}
              y1={y}
 ...
 */
export const ActiveSnapPointOverlay: React.FC<ActiveSnapPointOverlayProps> = ({
  snapPoint,
  viewport,
}) => {
