...
const getActivePlugin = (context: CanvasShortcutContext | null): string | null => {
  ...
  try {
    ...
    return typeof state === 'object' && state !== null && 'activePlugin' in state
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (state as any).activePlugin ?? null
      : null;
  } catch (_error) {
    return null;
  }
...
export class ShortcutRegistry {
  ...
  private normalizeKey(rawKey: string): string {
    ...
    return normalized.length === 1 ? normalized.toLowerCase() : normalized.toLowerCase();
  }
}
