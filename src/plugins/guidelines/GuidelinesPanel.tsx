...
const GuidelinesPanelComponent: React.FC = () => {
  ...
  const handleToggleAngleSnap = createToggleHandler('angleSnapEnabled');
  const handleToggleDebugMode = createToggleHandler('debugMode');
  
  const handleToggleSnapToEdges = createToggleHandler('snapToEdges');
  ...
  const handleAngleThresholdChange = (value: number) => {
    updateGuidelinesState?.({ angleSnapThreshold: value });
  };
        ...
        <VStack spacing={2} align="stretch">
          ...
          <PanelToggleGroup
            toggles={[
              ...
              { label: 'Angle', isChecked: guidelines?.angleSnapEnabled ?? false, onChange: handleToggleAngleSnap },
            ]}
            spacing={1}
          />
          ...
          <PanelToggleGroup
            toggles={[
              { label: 'Edge', isChecked: guidelines?.snapToEdges ?? true, onChange: handleToggleSnapToEdges },
          ...
          <Flex gap={2}>
            <NumberInput
              ...
              labelWidth="32px"
            />
            {guidelines?.angleSnapEnabled && (
              <NumberInput
                label="Ang"
                value={guidelines?.angleSnapThreshold ?? 5}
