...
class PanelRegistry {
  ...
  register(config: PanelConfig, position: 'start' | 'end' | number = 'end'): void {
    ...
    if (this.panels.has(config.key)) {
      console.warn(`Panel "${config.key}" is already registered. Skipping.`);
      return;
    }
...
const FilePanel = React.lazy(() => 
  import('../sidebar/panels/FilePanel').then(module => ({ default: module.FilePanel }))
);
const SettingsPanel = React.lazy(() => 
  import('../sidebar/panels/SettingsPanel').then(module => ({ default: module.SettingsPanel }))
);
const DocumentationPanel = React.lazy(() => 
  import('../sidebar/panels/DocumentationPanel').then(module => ({ default: module.DocumentationPanel }))
);
