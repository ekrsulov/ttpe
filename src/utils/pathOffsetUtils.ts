...
export function applyOffsetToPath(
  ...
  miterLimit: number = 4
): Omit<CanvasElement, 'id' | 'zIndex'> | null {
...
function offsetSubPathWithPaperJS(
  ...
  try {
    ...
    console.error('Error in offsetSubPathWithPaperJS:', error);
    return null;
  }
}
...
/**
 * Convert offset path back to path string
 */
export function offsetPathToString(offsetPath: CanvasElement): string | null {
  if (offsetPath.type !== 'path') return null;

  const pathData = offsetPath.data as PathData;
  const allCommands = pathData.subPaths.flat();
  return commandsToString(allCommands);
}
