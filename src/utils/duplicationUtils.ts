 ...
 */
import type { CanvasElement, GroupData, PathData, Command } from '../types';
...
function getElementBounds(
  ...
  if (element.type === 'path') {
    ...
    const bounds = calculateBounds(pathData.subPaths, pathData.strokeWidth || 0);
    ...
    const collectDescendantBounds = (group: CanvasElement) => {
      ...
      for (const childId of gData.childIds) {
        ...
        if (child.type === 'path') {
          // Paths have absolute coordinates - use them directly
          const pathData = child.data as PathData;
          const pathBounds = calculateBounds(pathData.subPaths, pathData.strokeWidth || 0);
          if (pathBounds) {
    ...
    // Return the combined bounds of all descendant paths
    // No need to add group transform - paths already have absolute coordinates
...
export function duplicateElement(
  ...
  if (element.type === 'group') {
    ...
    const tempGroupWithoutId: Omit<CanvasElement, 'id' | 'zIndex'> = {
      type: 'group',
    ...
    // Apply offset to path commands if offset is provided
    // Offset should only come from parent group if it was applied at root level
    // Paths store absolute coordinates, so we translate their commands directly
    const shouldApplyOffset = (offsetX !== 0 || offsetY !== 0);
    const subPaths = shouldApplyOffset
      ? pathData.subPaths.map((subPath: Command[]) =>
          translateCommands(subPath, offsetX, offsetY)
        )
      : pathData.subPaths;
    
    const newElementWithoutId: Omit<CanvasElement, 'id' | 'zIndex'> = {
      type: 'path',
      parentId: newParentId !== undefined ? newParentId : element.parentId,
      data: {
        ...pathData,
  ...
  throw new Error(`Unsupported element type for duplication`);
}
