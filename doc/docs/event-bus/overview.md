...
The **Canvas Event Bus** is a type-safe publish/subscribe (pub/sub) system that serves as the communication backbone of TTPE. It decouples plugins from direct canvas manipulation and enables loose coupling between components, allowing them to communicate without explicit dependencies.
...
TTPE's Event Bus handles five primary event types:
